...
namespace Hazel
{
	static GLenum ShaderTypeFromString(const std::string& type)
	{
		...
		HZ_CORE_ERROR("Undefined Shader Type");
		return 0 ;

	}
	OpenGLShader::OpenGLShader(const std::string& filePath)
	{
		std::string file = ReadShaderFromFile(filePath);
		auto Shaders = PreProcess(file);
		Compile(Shaders);
	...
	void OpenGLShader::UploadUniformMat3(const std::string& name, const glm::vec3& matrix)
	{
		GLint location = glGetUniformLocation(m_RendererID, name.c_str());
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			HZ_CORE_ASSERT(end != std::string::npos, "Syntax Error");
			size_t beg  = pos + tokenLength + 1;
 			std::string type = source.substr(beg,end - beg);
			...
			Shaders[ShaderTypeFromString(type)] = source.substr(next,pos - (next == std::string::npos ? source.size()- 1 : next));
		}
	...
	std::string OpenGLShader::ReadShaderFromFile(const std::string& filePath)
	{
		...
		{
			HZ_CORE_ERROR("Cant Open file '{0}'",filePath);
		}
		return result;
	}
	...
	void OpenGLShader::Compile(const std::unordered_map<GLenum, std::string>& shaders)
	{
		...
		HZ_CORE_ASSERT(shaders.size()<=2,"Only support 2 shaders");
